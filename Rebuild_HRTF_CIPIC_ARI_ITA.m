...
% antropométricos
%% Carregar dados
load('DADOS_TREINAMENTO/net_treinada_CIPIC_ARI_ITA.mat')
load('DADOS_TREINAMENTO/target_cipic_ari_ita.mat')
...
for n = 1:no_channels
    for i = 1:no_directions
        result{i, n} = net_CIPIC_ARI_ITA{i,n}(InptMtx(1:7,:, n));
    end
end
...
for k = 1:dim1 %azimute
    for l = 1:dim2  %elevação 
        hR = 10.^(squeeze(hrtf_sim(k, l, 1:no_samples/2, 1))./20);
        hL = 10.^(squeeze(hrtf_sim(k, l, 1:no_samples/2, 2))./20);
...
%  1 6 13 20 25
azz = 1; % azimute 1:25 % 13 representa 0º de azimute
...
semilogx(freq(1:Nt/2), h_prop_smoothL(1:Nt/2), '-.r', 'lineWidth', 1.5, 'Color', 'blue'); hold on
